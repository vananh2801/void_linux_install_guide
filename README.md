...
## Kết nối Wi-fi ở lần đầu khởi động bằng wpa_supplicant
...
3. Khởi chạy wpa_supplicant với interface là "wlp2s0" và file cấu hình mặc định:
...
- Thêm Wi-Fi:
  ...
  Chú ý kết quả trả về ở đây là số 0 nên các câu lệnh tiếp theo tôi dùng số 0.

  Nếu kết quả trả về là 1, 2, 3,... thì phải thay vào cho đúng.

- Thêm tên Wi-Fi:

  ```bash
  > set_network 0 ssid "<Tên Wi-Fi>"
  ```

- Thêm mật khẩu Wi-Fi:

  ```bash
  > set_network 0 psk "<Mật khẩu Wi-Fi>"
  ```

  Nếu Wi-Fi không có mật khẩu thì chạy lệnh:
...
3. Cài đặt fcitx5-lotus
   ...
   ```bash
   sudo xbps-install acl acl-progs cmake extra-cmake-modules libfcitx5-devel libinput-devel eudev-libudev-devel gcc go gettext-devel pkg-config hicolor-icon-theme libX11-devel python3-QtPy python3-PyQt5 python3-pyqt6 python3-pyqt6-gui python3-pyqt6-widgets librsvg-utils
...
6. Bật tự khởi chạy cho fcitx-lotus-server
